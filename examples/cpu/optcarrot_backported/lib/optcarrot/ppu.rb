  ...
  class PPU
    ...
    class OptimizedCodeBuilder
      ...
      def build
        ...
        splatted_args = []
        if @loglevel >= 3
          splatted_args << "  debug_logging(@scanline, @hclk, @hclk_target)"
        end
        splatted_args += [indent(2, code), "end"]
        gen("def self.run",*splatted_args)
      end
      ...
      def specialize_clock_handlers(handlers)
        handlers.each do |hclk, handler|
          ...
          handler = handler.gsub(/@hclk (==|>=) (\d+)/) { # "!=" undefined in 1.8.7
            hclk.send($1.to_sym, $2.to_i)
          }
      ...
      def build_loop(handlers)
        ...
        splat_arguments = clauses.invert.sort.map do |hclks, handler|
            "  when #{ hclks * ", " }\n" + indent(4, handler)
        end + [
          "  end",
          "end"
